...
complex,error,load}/ using openpyxl, so exceldiff is tested against files
...
def _tiny_png_bytes():
    ...
    entries (`many_images`): exceldiff never reads image bytes (Issue #65's
...
def _add_drawing_with_grouped_images(entries, image_path):
    ...
    `sample_image.png` bytes — exceldiff never reads them either way.
    """
...
def cell_hyperlinks():
    ...
    exceldiff's hand-authored fixtures happen to exercise, since they
...
def embedded_image():
    ...
    *across* a multi-cell range rather than confined to one, carrying both
    ...
    bytes, even though exceldiff itself never reads them (Issue #65's
...
def invalid_merge_ref():
    ...
    (unrelated, and not read by exceldiff at all) is deliberately left
