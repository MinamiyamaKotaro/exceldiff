...
/// (Issue #51). Unlike `<col min=".." max="..">`, `<row r="N" ht="..">` is
/// never itself a range in the OOXML schema — every row with an explicit
/// height gets its own `<row>` element — so `parse/worksheet.rs` is what
/// coalesces consecutive same-height rows into one `RowHeightRange` while
/// streaming `<sheetData>`, the same way `MergedRegion`/`ColWidthRange`
/// avoid a per-unit entry. A real grid-paper-style worksheet with a
/// uniform height stamped across every row of a 1,048,576-row sheet
/// collapses to a single range this way rather than one entry per row.
#[derive(Debug, Clone, Copy, PartialEq)]
...
impl Sheet {
    ...
    /// (Issue #50). Real workbooks commonly define a merged region's
    /// visible edge — e.g. the right border of `H2:Q2` — on the
    /// perimeter cell that touches that edge (`Q2`) rather than on the
    /// origin (`H2`) itself; Excel composites all of a merge's
    /// constituent cells when drawing its outline, but this model
    /// otherwise keeps only the origin's own style once merges are
    /// finalized, silently losing that edge. A cell not on a given edge
    /// (e.g. `coord.row` matching neither `region.start.row` nor
    /// `region.end.row`) never contributes its top/bottom to this fold —
    /// only borders that would actually land on the merged box's outline
    /// are considered.
...
mod tests {
    ...
    /// Issue #50: a real-world `.xlsx` (a "skill sheet" template) defined
    /// an `H2:Q2`-shaped merge's right border on `Q2` (the perimeter cell,
    /// `<right style="medium">` in `styles.xml`) rather than on the origin
    /// `H2` — a common Excel authoring pattern this sweep once silently
    /// dropped along with the rest of `Q2`'s own style. One-row merge:
    /// only the right edge is exercised (left/top already come from the
    /// origin itself for any merge, since a merge only ever extends
    /// right/down from its origin — see `MergedRegion`'s own doc comment).
    #[test]
