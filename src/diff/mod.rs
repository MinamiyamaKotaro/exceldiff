...
//! - `alignment`: `diff_workbooks_aligned_columns` (Issue #5) — the capped,
//!   opt-in alternative that matches columns by content first, so an
//!   inserted/deleted column doesn't cascade into spurious diffs for every
//!   column after it (see `alignment`'s doc comment for the full design).
