...
//!
//! ```text
//! xlsxdiff <display_path> <A|M|D> [base_file] [head_file]
//! ```
//!
...
use exceldiff::{diff_file_section_from_paths, MarkdownOptions};
use std::env;
...
fn main() -> ExitCode {
    ...
    let [_, display_path, status, rest @ ..] = args.as_slice() else {
        eprintln!("usage: xlsxdiff <display_path> <A|M|D> [base_file] [head_file]");
