...
GitHub Copilotの自動PRレビューが7ラウンドにわたり計21件の重大な問題を指摘し、いずれも検証の上で対応した（そのうち1件は過去ラウンドで既に評価・決定済みの論点の再指摘であり、新たなコード変更は不要と判断した——後述21番参照）:
...
19. **`mark_unique_headers`のO(cols²)コストが、非対称な列数のシートペアでは予算チェックに束縛されていなかった**: `mark_unique_headers`は重複ヘッダー検出のため全ペア総当たりでO(headered_cols²)の比較を行っていたが、そのdocコメントは「`align_columns`が既に予算化している同オーダーの作業」と正当化していた。しかし`align_sheet_columns`の予算チェックが束縛するのは`base_col_count * target_col_count`（積）であり、**片側だけの**列数の2乗ではない。そのため、例えば片側が1列、もう片側がヘッダー付き列を大量に持つような非対称なシートペアでは、積は小さく予算内に収まる一方、この関数自体のコストは「多い方の側」の列数だけで決まる純粋なO(n²)のままカタストロフィックに増大しうる。根本原因を除去する形で修正: `header: Option<&CellValue>`が常に`Text`であることを利用し、`HashMap<&str, usize>`でヘッダーテキストの出現回数を数えるO(headered_cols)の実装に書き換え、二次コストそのものを排除した（予算チェック側を拡張する対処ではなく）。
20. **`MAX_COLUMN_PAIR_COUNT`のメモリ見積もりが、`Score`がタプルになった後も更新されていなかった**: 18番の修正で`scores`/`dp`は`Vec<Vec<u64>>`ではなく`Vec<Vec<Option<Score>>>`/`Vec<Vec<Score>>`（`Score = (u64, u64)`）になったが、`MAX_COLUMN_PAIR_COUNT`のdocコメントは古い`u64`単体（8バイト/要素）ベースの見積もりのままだった。実測（`size_of::<Score>()` = 16バイト、`size_of::<Option<Score>>()` = 24バイト——`Option<(u64,u64)>`にはNoneを表す予備ビットが無く、判別子分の8バイトが追加されるため）に基づくと、既定値`1,000,000`ペアでの実際の合計メモリは約40MB（`scores`だけで約24MB）であり、docコメントが主張していた「約16MB」を2倍以上超過していた。docコメントを正しいバイト数計算に修正した上で、当初意図していた「約16MB」という予算を維持するため`MAX_COLUMN_PAIR_COUNT`を`400,000`に引き下げた。
21. **`CellDiff`の後方互換性破壊について、別のレビューパスから再度指摘があった**: 「`CellDiff`は全フィールドが`pub`な非`#[non_exhaustive]`構造体であり、`old_col`の追加は破壊的変更である」という指摘そのものは、[model.md 未決事項1](model.md)に記載の通り、既にラウンド5（本節14番）で評価済み・ユーザーとの合意の上で「既存の慣例（Issue #8）に従いバージョンは上げず、方針を明示的に記録する」と決定済みの論点である。GitHub Copilotの自動レビューは各パスが独立しており過去のレビュー結果の合意を認識しないため、同一の懸念が別のレビューIDから再度提起されたもの——内容を再確認した上で、新たな事実や論点の変化が無いことを確認し、既存の決定（バージョンは上げず、方針を明示的に文書化する）を維持した。追加のコード変更は行っていない。
