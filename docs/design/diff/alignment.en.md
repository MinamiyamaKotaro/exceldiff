...
Two rounds of GitHub Copilot's automated PR review flagged six real issues total, all confirmed and fixed:
...
- Formatting-only cells / `populated_count` (item 5 above): a shared blank range doesn't inflate the partial-match threshold (`formatting_only_blank_cells_do_not_inflate_the_partial_match_threshold`); a genuinely identical low-cardinality column with a shared blank cell still produces zero diff (`identical_low_cardinality_column_with_a_shared_blank_cell_still_produces_no_diff`); a large formatted-blank range doesn't push a real-data match out of threshold reach (`a_large_formatted_blank_range_does_not_raise_the_match_threshold_out_of_reach`)
