...
Design doc for `src/diff/engine.rs`. Computes the `WorkbookDiff` [`diff/model.rs`](model.en.md) defines from two `model::Workbook`s ([Issue #3](https://github.com/MinamiyamaKotaro/exceldiff/issues/3); style and merged-cell diffs added by [Issue #8](https://github.com/MinamiyamaKotaro/exceldiff/issues/8)).

## How the algorithm was chosen

[Issue #3](https://github.com/MinamiyamaKotaro/exceldiff/issues/3)'s PoC (`poc/issue3-poc`) implemented a "2D LCS alignment" (column LCS, then row LCS) that detects cell-coordinate shifts caused by row/column insertion or deletion. Building and running it confirmed it works correctly on small samples ([Issue #3 comment](https://github.com/MinamiyamaKotaro/exceldiff/issues/3#issuecomment-5382524419), `poc/issue3-poc/output/verification_report.md`). Benchmarking `align_columns`/`align_rows_2d` directly, however, measured a clear **O(distinct_rows² + distinct_cols²)** time/memory behavior:
...
- **Not responsible for**: the diff result type definitions ([`diff/model.rs`](model.en.md)), SQLite persistence ([`diff/storage.rs`](storage.en.md) — style/merge diffs are also persisted now, as of [Issue #9](https://github.com/MinamiyamaKotaro/exceldiff/issues/9)), the alignment-based diff that detects column insertion ([`diff/col_alignment.rs`](col_alignment.en.md), implemented for Issue #5), the alignment-based diff that detects row insertion ([`diff/row_alignment.rs`](row_alignment.en.md), implemented for Issue #4)
...
1. ~~Where/how a row/column-insertion alignment mode is implemented~~ → **Resolved for both**: columns were resolved in [Issue #5](https://github.com/MinamiyamaKotaro/exceldiff/issues/5), split into a separate submodule `diff::col_alignment` exposing `diff_workbooks_aligned_columns(base, target, limits: ColumnAlignmentLimits) -> Result<WorkbookDiff, Error>` (see [col_alignment.en.md](col_alignment.en.md)). Rows were resolved in [Issue #4](https://github.com/MinamiyamaKotaro/exceldiff/issues/4), split into a separate submodule `diff::row_alignment` exposing `diff_workbooks_aligned_rows(base, target, limits: RowAlignmentLimits) -> Result<WorkbookDiff, Error>` (see [row_alignment.en.md](row_alignment.en.md)).
2. ~~Robustness of the `similarity_score` heuristic~~ → **Resolved** ([Issue #5](https://github.com/MinamiyamaKotaro/exceldiff/issues/5)): five rounds of PoC investigation converged on replacing it with a `match_count ≥ max(2, 20% of column length)` threshold, plus a pre-scoring cardinality gate (`MIN_DISTINCT_FOR_CONTENT_MATCH`) that excludes extremely low-cardinality columns (2-4 distinct values) from content-based matching entirely. See [col_alignment.en.md](col_alignment.en.md)'s "Matching heuristic" section.
3. ~~Finer-grained style diffs~~ → **Resolved** ([Issue #8](https://github.com/MinamiyamaKotaro/exceldiff/issues/8)): added `CellDiff::old_style`/`new_style`, carrying fill color, font, borders, alignment, and number format (see this file's "Style diffs are sparser" section).
...
6. **Whether to add an option to exclude hidden (hidden/veryHidden) sheets from diffing** ([Issue #16](https://github.com/MinamiyamaKotaro/exceldiff/issues/16)): `diff_workbooks`/`diff_sheet` currently apply no `SheetVisibility` filter at all — `Hidden`/`VeryHidden` sheets are diffed for cells and merges exactly like `Visible` ones (parsing itself never excludes hidden sheets either, per `hidden_and_very_hidden_sheets_are_all_included` in `src/pipeline.rs`). This is a deliberate decision to keep the current "always diff every sheet" behavior, made in the absence of any actual request to exclude hidden sheets — should one arise (e.g. "hidden sheets are usually scratch work and add diff noise" vs. the opposite "hidden sheets are exactly where deliberately-concealed changes hide"), whether to add an opt-in filter (e.g. an argument controlling `hidden`/`veryHidden` independently) is to be decided then. `hidden_sheet_cell_changes_are_diffed_just_like_visible_ones` (`src/diff/engine.rs`) locks in the current "always included" contract as a regression test.
