...
4. Extract each row's content in a single true-O(cells) pass (`row_contents`) — relying on `iter_cells()` already yielding rows in ascending order, a row transition closes out the previous row's accumulator rather than bucketing through a `BTreeMap` (fixed in review on PR #21, the same class of issue as `distinct_row_count`'s). Each `RowContent` holds a `Vec` from column to cell, a content signature hashed with `RandomState` (a fresh, process-randomized seed, computed incrementally as cells are visited), and the real (non-formatting-only) cell count.
...
8. Any leftover contiguous Deleted/Inserted span Myers resolved but couldn't explain via an exact signature match is handed to `merge_leftover_spans_by_content_similarity`, which attempts content-similarity pairing — skipped (left as plain Deleted/Inserted) whenever the span exceeds `CONTENT_SIMILARITY_SPAN_CAP`, to avoid its O(span²) cost. The similarity computation itself (`row_similarity`) can run up to `CONTENT_SIMILARITY_SPAN_CAP²` times per span, so it's a merge-join over both sides' already column-ascending `RowContent::cells` rather than allocating a `HashMap` on every call (caught in review on PR #21).
