...
/// (`diff::engine::diff_workbooks`). `diff::col_alignment::
...
`CellDiff` gained both `old_col` (Issue #5, see [col_alignment.en.md](col_alignment.en.md)) and `old_row` (Issue #4, see [row_alignment.en.md](row_alignment.en.md)). Only `diff::col_alignment::diff_workbooks_aligned_columns` ever populates `old_col`, and only `diff::row_alignment::diff_workbooks_aligned_rows` ever populates `old_row` — `diff::engine::diff_workbooks` leaves both always `None`, and neither alignment mode populates the other's field, since `diff::col_alignment` never shifts rows and `diff::row_alignment` never shifts columns (using both together is not yet integrated — see [col_alignment.en.md Open Question 1](col_alignment.en.md)). `MergeDiff` still carries no separate `old_start`/`new_start` pair, because no engine detects a merge's origin coordinate shifting (see [engine.en.md](engine.en.md)/[col_alignment.en.md](col_alignment.en.md)/[row_alignment.en.md](row_alignment.en.md)).
...
1. ~~Type extension for a future row/column-alignment mode~~ → **Resolved for both**: columns were resolved in [Issue #5](https://github.com/MinamiyamaKotaro/exceldiff/issues/5), adding `CellDiff::old_col` (see [col_alignment.en.md](col_alignment.en.md)). Rows were resolved in [Issue #4](https://github.com/MinamiyamaKotaro/exceldiff/issues/4), adding `CellDiff::old_row` (see [row_alignment.en.md](row_alignment.en.md)). Both share the same `CellDiff` type `diff::engine::diff_workbooks` uses.

   **On versioning** (flagged by a Copilot review on the Issue #5 PR): `CellDiff` is a non-`#[non_exhaustive]` struct with every field `pub`, so adding `old_col` breaks any existing `CellDiff { .. }` struct-literal construction — a source-breaking change. The crate is currently `0.13.0`, but the same pattern (adding a field to `CellDiff`) in [Issue #8](https://github.com/MinamiyamaKotaro/exceldiff/issues/8) (`old_style`/`new_style`) didn't bump the version either, and this repository has no CHANGELOG. This PR follows that same established precedent and does not bump `Cargo.toml`'s version on its own — recorded here explicitly: version numbering is a separate decision the maintainer makes at publish (crates.io release) time, not per feature PR.
