...
6. **`CellDiff::old_col` の永続化**（[Issue #5](https://github.com/MinamiyamaKotaro/exceldiff/issues/5)）: `diff::alignment::diff_workbooks_aligned_columns` が追加した `CellDiff::old_col` は、`save_diff` では永続化されない——本ファイルの対象範囲は Issue #3/#8/#9 が要求した既存フィールドのみであり、`old_col` を `diff_records` へ追加するには項番5と同様の `ALTER TABLE ADD COLUMN` マイグレーションが必要になる。[model.md](model.md)参照。呼び出し側が `diff_workbooks_aligned_columns` の結果を永続化したい場合、現状は `WorkbookDiff` 自体を別途JSONとして保存する必要がある。
