...
- `save_diff`, which persists every cell diff in a `WorkbookDiff` (including `old_style`/`new_style`) as one row each in `diff_records`, and every merged-region diff as one row each in `merge_diff_records`, all inside a single transaction ([Issue #9](https://github.com/MinamiyamaKotaro/exceldiff/issues/9))
- `head_json`, which returns the `full_json` of whichever revision is currently flagged HEAD, verbatim
- **Not responsible for**: the diff computation itself ([`diff/engine.rs`](engine.en.md)), the diff result type definitions ([`diff/model.rs`](model.en.md)), querying/searching `diff_records`/`merge_diff_records` (the PoC in [Issue #3](https://github.com/MinamiyamaKotaro/exceldiff/issues/3) had a query capability, but it goes beyond the user's explicit request scope — see Open Question 2), persisting `SheetDiff::status`/`old_visibility`/`new_visibility` (sheet-level visibility changes/additions/deletions — see Open Question 3; a `sheet_diff_records` table was proposed during [Issue #9](https://github.com/MinamiyamaKotaro/exceldiff/issues/9)'s extended PoC review but deferred as out of Issue #9's own scope, which was limited to `old_style`/`new_style`/`merges`, and split off into [Issue #14](https://github.com/MinamiyamaKotaro/exceldiff/issues/14))
...
- ([Issue #9](https://github.com/MinamiyamaKotaro/exceldiff/issues/9)) when `CellDiff::old_style`/`new_style` is set, `diff_records.old_style_json`/`new_style_json` stores exactly `serde_json::to_string`'s own output
- ([Issue #9](https://github.com/MinamiyamaKotaro/exceldiff/issues/9)) a `Modified` cell whose value changed but whose style didn't stores `NULL` in both `old_style_json`/`new_style_json` (never an empty string or a fabricated style)
- ([Issue #9](https://github.com/MinamiyamaKotaro/exceldiff/issues/9)) each `Added`/`Modified`/`Deleted` entry in `SheetDiff::merges` becomes one `merge_diff_records` row, with `kind`/`start_row`/`start_col`/`old_end_row`/`old_end_col`/`new_end_row`/`new_end_col` matching
- ([Issue #9](https://github.com/MinamiyamaKotaro/exceldiff/issues/9)) calling `DiffStore::open` on a real file carrying a pre-[Issue #9](https://github.com/MinamiyamaKotaro/exceldiff/issues/9) schema (`diff_records` missing the style columns, no `merge_diff_records` table) adds the missing columns/table while preserving the pre-existing row (see Migration Policy above)
