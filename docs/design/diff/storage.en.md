...
6. **Persisting `CellDiff::old_col`** ([Issue #5](https://github.com/MinamiyamaKotaro/exceldiff/issues/5)): `diff::alignment::diff_workbooks_aligned_columns`'s new `CellDiff::old_col` field is not persisted by `save_diff` — this file's scope is the existing fields Issues #3/#8/#9 asked for, and adding `old_col` to `diff_records` would need the same kind of `ALTER TABLE ADD COLUMN` migration as item 5. See [model.en.md](model.en.md). A caller that wants to persist `diff_workbooks_aligned_columns`'s output today has to save the `WorkbookDiff` as JSON separately.
