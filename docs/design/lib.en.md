...
/// [#14](https://github.com/MinamiyamaKotaro/exceldiff/issues/14)).
...
/// review](https://github.com/MinamiyamaKotaro/exceldiff/pull/11#pullrequestreview-4949346233)).
...
`SizeLimits`'s re-export relies on the same Rust visibility rule: `container::sanitize` is declared `pub mod sanitize;` inside [container/mod.rs](container/mod.en.md), so `SizeLimits` itself is a `pub` type reachable from anywhere in the crate — but since the enclosing `container` module is declared `mod container;` (private) in this file, the outside world has no path `exceldiff::container::sanitize::SizeLimits` to reach it through. Only this file's `pub use container::sanitize::SizeLimits;` opens the single public path `exceldiff::SizeLimits`. `DEFAULT_MAX_UNCOMPRESSED_SIZE` / `DEFAULT_MAX_TOTAL_UNCOMPRESSED_SIZE` are not included in this re-export — `SizeLimits::default()` already exposes both values, so there is no need to expose the raw constants as part of the public API too.
...
2. ~~Crate name / package name~~ → **Resolved**: `exceldiff` (set in `Cargo.toml` when the crate skeleton was scaffolded for CI, Issue #16, ahead of this file's implementation).
