...
A bare major-version tag like `v1` (matching this action's own README usage example, `uses: MinamiyamaKotaro/exceldiff@v1`) is treated as a *moving* pointer — following the convention popular Actions like `actions/checkout` use, the `v1` tag gets force-moved to the same commit and re-pushed every time a new `v1.x.y` is released. That re-push re-triggers `release.yml`, but `gh release create` fails outright if a release already exists for that tag, so the "Create GitHub Release" step is made idempotent: it first checks whether a release for `$TAG` already exists via `gh release view`, and if so, deletes just the release *object* with `gh release delete "$TAG" --yes` (the git tag itself is untouched unless `--cleanup-tag` is also passed) before recreating it. When the tag shape matches `^v[0-9]+$` (a bare major version), `--generate-notes` is skipped too — it diffs against "the previous release," which for a tag that keeps getting recreated in place would be a different, confusing diff every time — in favor of fixed, static `--notes` instead.
