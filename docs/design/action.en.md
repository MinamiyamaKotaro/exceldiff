...
runs:
  ...
  steps:
    ...
    - id: diff               # for each changed file: git show + run xlsxdiff, assembling
                               ...
                               # $GITHUB_OUTPUT. If visual: true, also collects each
                               # sheet's HTML page into ${{ runner.temp }}/xlsx-diff-visuals/
                               # and writes has-visuals to $GITHUB_OUTPUT
...
1. **Collecting the standalone HTML page**: each sheet's HTML — written by `xlsxdiff --grid-html-dir <dir>`, already wrapped via `exceldiff::wrap_grid_page` (self-contained: inline `<style>` only, no external assets) — is copied as-is to `${{ runner.temp }}/xlsx-diff-visuals/{sanitize(file path)}/sheet-{sanitize(sheet name)}.html` (`sanitize` collapses anything outside alphanumerics/`._-` to `_`). Never committed to `git`. The diff step's `has-visuals` output flips to `true` once at least one page has been collected.
2. **Uploaded as a single artifact**: once every changed file has been processed, and only if `has-visuals == 'true'`, `actions/upload-artifact@v4` uploads the whole `xlsx-diff-visuals/` directory as one artifact (`xlsx-diff-grids`) — once per job run, not once per sheet.
