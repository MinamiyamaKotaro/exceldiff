...
1. ~~**Delivery path for the generated HTML**~~ **Resolved**: implemented as a follow-up to [Issue #24](https://github.com/MinamiyamaKotaro/exceldiff/issues/24). CI renders a screenshot PNG via Playwright (headless Chromium). It was originally committed and pushed to a dedicated orphan branch (`xlsx-diff-images`) and embedded in the PR comment as a Markdown image via its `raw.githubusercontent.com` URL, but that left screenshots unviewable on private repositories, so it was replaced with uploading the screenshot as a workflow artifact (`actions/upload-artifact@v4`) and posting a download link in the comment instead ([Issue #47](https://github.com/MinamiyamaKotaro/exceldiff/issues/47); `action.yml`'s `visual` input, see [action.md](action.en.md) for the full design).
