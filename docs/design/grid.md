...
4. ~~**結合セルの右/下罫線が消える**~~ **解決済み(Issue #50)**: 冒頭で謳っている「実際のExcelのグリッドを忠実に再現」に反し、結合範囲の外周(特に右端・下端)の罫線が実際のExcelファイルで欠落するケースがあった。原因は本モジュールではなく[`model/sheet.md`](model/sheet.md)の`Sheet::finalize_merges`側にあった——結合範囲内の非起点セル自身が持つ罫線情報が、パース完了時点で無条件に破棄されていた(詳細は[model/sheet.md](model/sheet.md)「訂正(Issue #50)」節参照)。`grid.rs`自体のコードは無変更のまま、モデル層の修正だけで本モジュールの描画結果も正しくなった。
