...
- While processing each `<row r="N" ht="..">` inside `<sheetData>`, calls `push_row_height` right there if `ht` is present, compressing into `Vec<RowHeightRange>` as it goes (Issue #51) — unlike `<col>`, `<row>` can never itself express a range in the OOXML schema (always one element per row), so reading pre-compressed ranges the way `<cols>` allows isn't an option: **the compression itself is this file's own responsibility**, the decisive difference from column width (extends the last range's `max` when the current row immediately follows it at the same height, otherwise starts a new one — see [resolve/row_height.en.md](../resolve/row_height.en.md) for the full rationale and measurements). Also collects `<sheetFormatPr defaultRowHeight="..">` if present. Validating and registering the already-compressed `Vec<RowHeightRange>` itself is [`resolve/row_height.rs`](../resolve/row_height.en.md)'s job
...
- `width`/`defaultColWidth`/`ht`/`defaultRowHeight` all go through the same shared `parse_f64_attr`, which checks not just that the value parses as `f64` but that it's **finite and non-negative** — `NaN`, `±Infinity`, and negative numbers are all rejected as `Error::InvalidPackage`. Without this, `grid.rs`'s pixel conversion (`excel_width_to_px`/`excel_height_pt_to_px`) feeds the value straight into Rust's `as u32` cast, which would silently saturate a non-finite or negative size to `0`/`u32::MAX` instead of surfacing the bad file (a Copilot review on PR #53 caught this; the fix protects column width the same way, since it shares the helper)
...
- Verify the compression algorithm itself (Issue #51, `push_row_height`): consecutive same-height rows compress into one range; a gap in row numbers (a row with no explicit `<row ht>` in between) starts a new range even at the same height; a row with no `ht` attribute contributes nothing
- Verify that a malformed `<col>`/`<sheetFormatPr>` numeric attribute returns `Error::InvalidPackage`
- Verify that `NaN`, `inf`, `-inf`, and a negative number (`-1.0`) are all rejected as `Error::InvalidPackage` for each of `ht`, `defaultRowHeight`, and `width` (a Copilot review on PR #53; added as a regression test since column width shares the same `parse_f64_attr`)
