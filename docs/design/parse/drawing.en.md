...
7. ~~Whether `<xdr:grpSp>` nesting depth needs a defensive bound~~ → **Resolved**: it does. `resolve_grouped_pic` costs O(current nesting depth) per `<xdr:pic>` resolved, so a drawing part with `D` levels of nesting and `N` sibling pictures at the innermost level costs O(N × D) while costing only O(N + D) bytes to construct — the Zip Bomb byte-size cap alone does not bound `D`, the same shape xlsxparser's own [`docs/security/old/code-review.en.md`](https://github.com/MinamiyamaKotaro/xlsxparser/blob/master/docs/security/old/code-review.en.md) Finding 1 found for merge-cell count. Measured (xlsxparser's own security review — this fix landed in the shared parser code, so the finding and its measurements live in [xlsxparser's `docs/security/design-review.en.md`](https://github.com/MinamiyamaKotaro/xlsxparser/blob/master/docs/security/design-review.en.md)/[`code-review.en.md`](https://github.com/MinamiyamaKotaro/xlsxparser/blob/master/docs/security/code-review.en.md) Finding 1 there, not in exceldiff's own `docs/security/`, which is scoped to exceldiff-specific code only): a 22.6 MB drawing part (D=N=50,000) took 10.9s of synchronous CPU time before the fix. Added `MAX_GROUP_NESTING_DEPTH = 64` (`parse::drawing`), checked at each `<xdr:grpSp>` start tag — the same defensive-cap pattern `resolve::merge::MAX_MERGE_REGIONS`/`resolve::column_width::MAX_COLUMN_WIDTH_RANGES` already establish. Re-measured post-fix: the identical attack shape now rejects in under 1ms. A related, separate numeric-magnitude bound (`MAX_PLAUSIBLE_EMU`) was added at the same time — see Error Handling Policy above and xlsxparser's own `docs/security/design-review.en.md`/`code-review.en.md` Findings 1–2 for full detail.
