...
impl<'a> Serialize for JsonSheet<'a> {
    fn serialize<S: Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        ...
        // `defaultRowHeight`/`rows` (Issue #51): the same sheet-level-array
        // design as `columns`, applied symmetrically to row height.
...
- **Verify that `Sheet::row_height_ranges`/`default_row_height` serialize the same way as a sheet-level `rows` array / `defaultRowHeight` field, never duplicated onto individual cell objects** (Issue #51; the same design decision, symmetric with `columns`)
