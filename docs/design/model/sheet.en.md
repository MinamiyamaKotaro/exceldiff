...
**Correction (Issue #50): "never discards observable data" was inaccurate for borders.** The claim above is correct about *reachability* — a non-origin coordinate was already unreachable through `get`/`iter_cells` — but it missed that a non-origin cell's own border style isn't necessarily dead data: it can still be the information that should contribute to the merged region's visible outline. A real-world `.xlsx` (a "skill sheet" template) demonstrated this concretely: Excel had recorded an `H2:Q2`-shaped merge's right border as `<right style="...">` on the rightmost *non-origin* cell (`Q2`) rather than on the origin (`H2`), and that border went missing from both `grid.rs`'s grid rendering and `json.rs`'s JSON output once `finalize_merges` dropped `Q2`. The fix: right before a non-origin cell is pushed onto `to_drop`, `fold_perimeter_border_into_origin` checks whether it sits on the region's outer edge (top row, left column, bottom row, right column) and, for each edge it touches, ORs that side's border into the origin's own `Borders` — a cell that touches no edge (interior to the merge) never contributes, since its border couldn't have been part of the visible outline anyway. Because the origin's style is a `Option<Arc<ResolvedStyle>>` potentially shared with other cells, the update goes through `Arc::make_mut` (clones only if the reference count isn't 1) rather than mutating through the existing `Arc` directly — otherwise the border would spuriously leak onto every other cell sharing that same style (regression test `finalize_merges_border_fold_never_mutates_a_style_shared_with_unrelated_cells`). The fold-in is O(1) per covered cell inside the existing `SweepEvent::Query` match arm, so the sweep's own O((C + M) log (C + M)) bound is unchanged.

## Relationship to the sister project `xlsxparser` (Issue #50)

`xlsxparser` (`src/model/sheet.rs`) turned out to have byte-for-byte identical `resolve_origin`/`finalize_merges` implementations — this border-dropping behavior is a `Sheet`-model-layer trait shared by both projects, not an exceldiff-specific bug. `grid.rs` (the module that actually renders merged cells visually) only exists in exceldiff, so this particular fix is self-contained there, but keeping `model/sheet.rs`'s own design consistent across both repositories means the identical logic change is worth mirroring into `xlsxparser` too (as a separate, follow-up PR in that repository).
...
- **Verifying `finalize_merges` folds a merged region's outer-edge borders into its origin** (Issue #50): both a single-row merge (only the right edge is exercised) and a genuinely 2D merge (all four edges, each carried by a different cell, plus an interior cell that must *not* contribute)
- **Verifying that fold-in never mutates a `Arc<ResolvedStyle>` shared with an unrelated cell** (Issue #50; regression coverage that the `Arc::make_mut` COW split actually works)
