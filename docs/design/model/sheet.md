...
**訂正(Issue #50): 「観測可能なデータを一切失わない」は罫線について不正確だった。** 上記の主張は、非起点セルが`get`/`iter_cells`からもともと到達不能だった、という**到達可能性**の意味では正しいが、非起点セル自身の罫線情報がそもそも「死んでいたデータ」ではなく、結合範囲の見た目の外枠に寄与すべき情報だったケースを見落としていた。実データ(`.xlsx`の「スキルシート」テンプレート)で確認したところ、`H2:Q2`のような結合セルの右罫線をExcelが**右端の非起点セル(Q2)自身**の`<right style="...">`として保持している例が実在し、これは`grid.rs`のグリッド描画と`json.rs`のJSON出力の両方で右罫線が欠落するという形で顕在化した。対処として、`finalize_merges`が非起点セルを`to_drop`へ積む直前に`fold_perimeter_border_into_origin`を呼び、そのセルが結合範囲の外周(上端行/左端列/下端行/右端列)に位置する場合は該当する辺の罫線を起点セルの`Borders`へOR演算で畳み込んでから削除するようにした——結合範囲の内部(どの辺にも接しない)セルの罫線は外枠に寄与しないため対象外。起点セルのスタイルは`Option<Arc<ResolvedStyle>>`で他のセルと共有されうるため、既存のArcを直接書き換えず`Arc::make_mut`(参照カウントが1でなければ内部でクローンしてから可変参照を返す)で分離してから更新する——さもないと同じスタイルを共有する無関係なセルにまで罫線が波及してしまう(回帰テスト`finalize_merges_border_fold_never_mutates_a_style_shared_with_unrelated_cells`)。この畳み込みは`SweepEvent::Query`の一致判定内で行毎セルにつきO(1)であり、スイープライン全体の計算量O((C + M) log (C + M))は変わらない。

## 姉妹プロジェクト`xlsxparser`との関係(Issue #50)

`xlsxparser`(`src/model/sheet.rs`)の`resolve_origin`/`finalize_merges`はexceldiffと一言一句同じ実装であることを確認した——この罫線欠落は`Sheet`モデル層に共通する挙動であり、exceldiff固有のバグではない。`grid.rs`(結合セルを画面描画する機能)自体はexceldiffにしか存在しないため、この修正の適用先はexceldiff側だけで完結するが、`model/sheet.rs`自体の設計としての一貫性を保つには、xlsxparser側にも同一のロジックを反映することが望ましい(別リポジトリでの追従PRとして対応)。
...
- **`finalize_merges` が結合範囲の外周罫線を起点セルへ畳み込むことの確認**（Issue #50。1行結合(右端のみ)・2次元結合(4辺すべてを別々のセルが担うケース、内部セルは対象外であることを含む)の両方を検証)
- **その畳み込みが、他の無関係なセルと共有された`Arc<ResolvedStyle>`を書き換えないことの確認**（Issue #50。`Arc::make_mut`によるCOW分離が実際に機能していることの回帰テスト）
