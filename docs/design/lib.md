...
`SizeLimits` の再エクスポートも同じ Rust の可視性規則を利用する: `container::sanitize` は [container/mod.rs](container/mod.md) 内で `pub mod sanitize;` と宣言されているため `SizeLimits` 自体はクレート内のどこからでも到達可能な `pub` 型だが、それを包含する `container` モジュールが本ファイルで `mod container;`（非公開）と宣言されている以上、クレート外部は `exceldiff::container::sanitize::SizeLimits` という経路では到達できない。本ファイルが `pub use container::sanitize::SizeLimits;` を書くことで初めて `exceldiff::SizeLimits` という単一の公開経路が開く。`DEFAULT_MAX_UNCOMPRESSED_SIZE` / `DEFAULT_MAX_TOTAL_UNCOMPRESSED_SIZE` の各定数はこの再エクスポートに含めない（`SizeLimits::default()` が既に両定数の値をそのまま提供するため、定数自体を公開APIとして露出する必要性がない）。
...
- 公開型（`Workbook`, `Sheet`, `Cell`, `CellValue`, `CellRef`, `SheetVisibility`, `MergedRegion`, `ResolvedStyle`, `StyleId`, `DateTimeValue`, `SizeLimits`, `Error`, `Result`）がクレート外部から `exceldiff::` 直下の名前として参照可能であることの確認（doctest、または公開API surfaceを固定するテスト。具体的な手法はオープンクエスチョン参照）
...
2. ~~クレート名・パッケージ名~~ → **解決**: `exceldiff`（本ファイルの実装に先立ち、CI整備のためのクレート雛形を作った際に `Cargo.toml` で確定済み。Issue #16）。
